...
DROP POLICY IF EXISTS "Users select own letter stats" ON public.letter_stats;
DROP POLICY IF EXISTS "Users insert own letter stats" ON public.letter_stats;
DROP POLICY IF EXISTS "Users update own letter stats" ON public.letter_stats;
DROP POLICY IF EXISTS "Users delete own letter stats" ON public.letter_stats;
DROP POLICY IF EXISTS "Admins view all letter stats" ON public.letter_stats;

CREATE POLICY "Users select own letter stats"
ON public.letter_stats
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Users insert own letter stats"
ON public.letter_stats
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users update own letter stats"
ON public.letter_stats
FOR UPDATE
TO authenticated
USING (auth.uid() = user_id)
...
CREATE POLICY "Users delete own letter stats"
ON public.letter_stats
FOR DELETE
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Admins view all letter stats"
ON public.letter_stats
FOR SELECT
TO authenticated
...
DROP POLICY IF EXISTS "Users delete own answer events" ON public.answer_events;
...
CREATE POLICY "Users delete own answer events"
ON public.answer_events
FOR DELETE
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Admins view all answer events"
ON public.answer_events
FOR SELECT
TO authenticated
USING (public.has_role(auth.uid(), 'admin'::public.app_role));
...
CREATE OR REPLACE FUNCTION public.record_letter_answer(
  ...
  _letter_id text,
  _correct boolean,
  _game_id text DEFAULT NULL::text,
  _response_ms integer DEFAULT NULL::integer,
